# Contributing to Blackboard LMS Browser Automation
...
- **Broken selectors**: Blackboard LMS updates its UI periodically. If a selector breaks, open an issue with the current selector and what it should be.
...
2. Make your changes
3. Test your changes against a real Blackboard LMS instance (if you have access)
...
async function my_new_action(page, opts = {}) {
  ...
  return retry(async () => {
    ...
    await page.goto(`${process.env.BLACKBOARD_URL}/path/to/section`);
...
When you find working selectors for Blackboard LMS, document them:

```javascript
// Blackboard LMS selectors (verified YYYY-MM-DD)
...
Blackboard LMS updates its UI periodically. When selectors break:
