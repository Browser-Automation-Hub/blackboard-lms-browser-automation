/**
 * auth.js — Authentication handler for Blackboard LMS
 * Supports: SSO (SAML / CAS / Shibboleth), MFA (Duo / Microsoft MFA)
 *
...
const BASE_URL = process.env.BLACKBOARD_URL || 'https://your-school.blackboard.com/webapps/login/';
const USERNAME = process.env.BLACKBOARD_USERNAME;
const PASSWORD = process.env.BLACKBOARD_PASSWORD;
...
async function performLogin(page) {
  if (!USERNAME || !PASSWORD) {
    throw new Error(`Set BLACKBOARD_USERNAME and BLACKBOARD_PASSWORD in .env`);
  }

  console.log('Logging in to Blackboard LMS...');
  await page.goto(BASE_URL, { waitUntil: 'networkidle2' });
