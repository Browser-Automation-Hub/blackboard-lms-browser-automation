/**
 * anchor-cloud.js — AnchorBrowser cloud example for Blackboard LMS
 *
...
const { login_blackboard, enroll_students } = require('../src/actions');
...
async function main() {
  ...
  try {
    log('Starting Blackboard LMS automation (AnchorBrowser Cloud mode)');
    ...
    const result = await withAnchorBrowser(async (page) => {
      ...
      const data1 = await login_blackboard(page, {});
      log('Action 1 result:', data1);

      const data2 = await enroll_students(page, {});
